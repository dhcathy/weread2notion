...
export async function getBookProgress(
  ...
  try {
    ...
    if (response.data && response.data.book) {
      ...
      // 评分信息
      if (book.newRating) {
        console.log(`newRating (评分): ${book.newRating}星`);
      }
      if (book.newRatingDetail && book.newRatingDetail.myRating) {
        console.log(`myRating (我的评分): ${book.newRatingDetail.myRating}星`);
      }
